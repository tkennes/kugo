...
type Io_k8s_api_admissionregistration_v1_MutatingWebhook struct {
	...
	// See: file:///Users/tomkennes/Documents/Clients/_generic/kugo/model/Io_k8s_api_admissionregistration_v1_WebhookClientConfig.go
	...
	// "matchExpressions": [     {       "key": "runlevel",       "operator": "NotIn",       "values": [         "0",
	// "1"       ]     }   ] }  If instead you want to only run the webhook on any objects whose namespace is associated with
	// the "environment" of "prod" or "staging"; you will set the selector as follows: "namespaceSelector": {
	...
	// that specify this option *must* be idempotent, able to process objects they previously admitted. Note: * the number of
	// additional invocations is not guaranteed to be exactly one. * if additional invocations result in further modifications
	// to the object, webhooks are not guaranteed to be invoked again. * webhooks that use this option may be reordered to
	// minimize the number of additional invocations. * to validate an object after all mutations are guaranteed complete, use
	// a validating admission webhook instead.  Defaults to "Never".
	// See: file:///Users/tomkennes/Documents/Clients/_generic/kugo/model/string.go
	...
	// See: file:///Users/tomkennes/Documents/Clients/_generic/kugo/model/Io_k8s_api_admissionregistration_v1_RuleWithOperations.go
